...
function UserFooter({ collapsed }: { collapsed: boolean }) {
    ...
    if (collapsed) {
        return (
            <div className="px-2 py-3 border-t border-nav-border shrink-0 flex justify-center">
                <button
    ...
    return (
        <div ref={ref} className="relative border-t border-nav-border shrink-0">
            {/* Dropdown menu - slides up above the footer */}
            {open && (
                <div className="absolute bottom-full left-2 right-2 mb-1 bg-nav-hover rounded-xl border border-nav-border shadow-xl overflow-hidden z-50">
                    <div className="px-4 py-3 border-b border-nav-border">
                        <p className="text-xs font-semibold text-white truncate">{fullName}</p>
                        <p className="text-2xs text-white/60 truncate mt-0.5">{user.email}</p>
                        {roleName && (
                    ...
                    <ul className="py-1">
                        <li>
                            <button
                                ...
                                className="w-full flex items-center gap-3 px-4 py-2.5 text-xs text-white/80 hover:bg-nav-hover hover:text-white transition-colors"
                            >
                        ...
                        <li className="border-t border-nav-border mt-1 pt-1">
                            <button
            ...
            <button
                onClick={() => setOpen((v) => !v)}
                className={clsx(
                    "w-full flex items-center gap-3 px-4 py-3 hover:bg-nav-hover transition-colors group",
                    open && "bg-nav-hover"
                )}
            >
                ...
                <div className="flex-1 min-w-0 text-left">
                    ...
                    <p className="text-2xs text-white/60 truncate">{user.email}</p>
                    {roleName && (
                        <p className="text-2xs text-brand-400/80 truncate mt-0.5">{roleName}</p>
                    )}
                </div>
                <svg
                    className={clsx("w-3.5 h-3.5 text-white/60 shrink-0 transition-transform", open ? "rotate-180" : "")}
...
export function Sidebar({ collapsed }: SidebarProps) {
    ...
    return (
        <aside
            className={clsx(
                "flex flex-col h-full bg-nav text-white transition-all duration-300 ease-spring",
                collapsed ? "w-16" : "w-64",
            )}
        >
            {/* Logo */}
            <div
                className={clsx(
                    "h-[60px] flex items-center border-b border-nav-border shrink-0 transition-all duration-300",
                        ...
                        <div key={group.label} className="mb-1">
                            ...
                            {!collapsed && (
                                <button
                                    ...
                                    className="w-full flex items-center justify-between px-4 py-1.5 text-xs font-semibold tracking-widest uppercase text-white/55 hover:text-white transition-colors"
                                >
                                    {group.label}
                                    <svg
                                        className={clsx(
                                                ...
                                                <Link
                                                    ...
                                                    className={clsx(
                                                        ...
                                                        isActive
                                                            ...
                                                            : "text-white hover:bg-nav-hover hover:text-white",
                                                    )}
                                                >
