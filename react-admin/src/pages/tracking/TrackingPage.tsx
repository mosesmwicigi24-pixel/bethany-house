 ...
 *   - bg-surface-50 page, white rounded-2xl cards, border border-surface-100 shadow-sm
...
const STATUS_CONFIG: Record<string, { label: string; tw: string; bar: string }> = {
    order_confirmed:    { label: "Order Confirmed",    tw: "text-info-600 bg-info-50 border-info-200",    bar: "bg-info-600"   },
    processing:         { label: "Processing",         tw: "text-accent-600 bg-accent-50 border-accent-200", bar: "bg-accent-600" },
    ready_to_ship:      { label: "Ready to Ship",      tw: "text-info-700 bg-info-100 border-info-300",    bar: "bg-info-700"   },
    picked_up:          { label: "Picked Up",          tw: "text-info-900 bg-info-200 border-info-400",    bar: "bg-info-800"   },
    in_transit:         { label: "In Transit",         tw: "text-amber-600 bg-amber-50 border-amber-200",  bar: "bg-amber-500"  },
    out_for_delivery:   { label: "Out for Delivery",   tw: "text-brand-600 bg-brand-50 border-brand-200", bar: "bg-brand-500" },
    delivery_attempted: { label: "Delivery Attempted", tw: "text-danger-600 bg-danger-50 border-danger-200",        bar: "bg-danger-500"    },
    delivered:          { label: "Delivered",          tw: "text-success-700 bg-success-50 border-success-200",  bar: "bg-success-600"  },
    exception:          { label: "Exception",          tw: "text-danger-900 bg-danger-200 border-danger-400",       bar: "bg-danger-700"    },
    cancelled:          { label: "Cancelled",          tw: "text-surface-500 bg-surface-100 border-surface-200",    bar: "bg-surface-400"   },
};
...
function SectionLabel({ children }: { children: React.ReactNode }) {
    return (
        <p className="text-[10px] font-bold tracking-widest uppercase text-surface-400 mb-3">
            {children}
        </p>
    );
}
                        ...
                        <div key={m.status} className="flex-1 flex flex-col items-center">
                            ...
                            <div className="flex items-center w-full h-6">
                                ...
                                <div
                                    ...
                                    style={{ background: i === 0 ? "transparent" : (isDone || isActive) ? hex : "#e7eae6" }}
                                />
                                {/* Dot */}
                                <div
                                    ...
                                    style={{
                                        ...
                                        background: isDone || isActive ? hex : "#e7eae6",
                                ...
                                <div
                                    className="flex-1 h-0.5 transition-colors duration-300"
                                    style={{ background: isLast ? "transparent" : isDone ? hex : "#e7eae6" }}
                                />
                            </div>
                            {/* Label */}
                            <span
        ...
        >
            ...
            {isImage ? (
                ...
                >
                    ...
                    <p className="text-xs text-surface-400">No inline preview available for this file type.</p>
                </div>
            )}
        </div>
    );
...
function FileTypeIcon({ mimeType }: { mimeType?: string | null }) {
    if (mimeType === "application/pdf") {
        return (
            <svg className="w-4 h-4 text-danger-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
            </svg>
        );
    }
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                {files.map((a, i) => (
                    <button
                        key={`file-${i}`}
                        onClick={() => setPreviewing(a)}
                        className="inline-flex items-center gap-1.5 text-xs font-medium text-info-600 hover:underline bg-info-50 rounded-lg px-2.5 py-1.5"
                    >
    ...
    return (
        <div>
            {reversed.map((e, i) => {
                ...
                const hex     = STATUS_HEX[e.status] ?? "#757d72";
                        ...
                        <div className="flex flex-col items-center">
                            <div
                                ...
                                style={{
                                    width:     isFirst ? 10 : 8,
                                    height:    isFirst ? 10 : 8,
                                    background: isFirst ? hex : "#d2d6d1",
                                    boxShadow:  isFirst ? `0 0 0 3px ${hex}18` : "none",
                                }}
                            />
                            {!isLast && <div className="flex-1 w-px bg-surface-100 mt-1.5" />}
                        </div>
                        ...
                        <div className="pt-0.5 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                <span
                                    ...
                                    style={{ color: isFirst ? hex : "#8c9489" }}
                                >
                                    {e.label ?? e.status}
                                </span>
                                {e.location && (
                            ...
                            {e.description && (
                                <p className={clsx("text-sm leading-relaxed", isFirst ? "text-surface-700" : "text-surface-400")}>
                                    {e.description}
                                </p>
                            )}
...
function QuerySection({ token }: { token: string }) {
    ...
    if (done) {
        return (
            <div className="flex flex-col items-center py-6 gap-3">
                <div className="w-11 h-11 rounded-full bg-success-50 flex items-center justify-center">
                    <svg className="w-5 h-5 text-success-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                </div>
                <div className="text-center">
                    <p className="text-sm font-semibold text-surface-800">Message sent</p>
                    <p className="text-xs text-surface-400 mt-0.5">We'll be in touch shortly.</p>
                </div>
            </div>
        );
    }
    ...
    if (!open) {
        return (
            <button
                ...
                className="flex items-center gap-2 text-sm text-surface-400 hover:text-surface-600 transition-colors group"
            >
                <div className="w-8 h-8 rounded-xl bg-surface-100 group-hover:bg-surface-200 flex items-center justify-center transition-colors shrink-0">
    ...
    const inputCls = "w-full border border-surface-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-info-500 focus:border-transparent";

    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-surface-800">Send us a message</p>
                <button
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-surface-700">Your name</label>
                    <input value={name} onChange={e => setName(e.target.value)} placeholder="Jane Doe" className={inputCls} />
                </div>
                <div className="space-y-1.5">
                    <label className="block text-xs font-semibold text-surface-700">Email</label>
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="jane@example.com" className={inputCls} />
                </div>
            </div>

            <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-surface-700">Message</label>
                <textarea
                    value={message}
            ...
            <button
                ...
                className="w-full py-2.5 px-5 rounded-xl text-sm font-semibold bg-surface-900 text-white hover:bg-surface-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
            >
                {mutation.isPending
...
function PageShell({ children }: { children: React.ReactNode }) {
    return (
        <div className="min-h-screen bg-surface-50 font-sans antialiased text-surface-900">
            {children}
        </div>
    );
}
        ...
        <PageShell>
            <div className="min-h-screen flex items-center justify-center">
                <div className="flex flex-col items-center gap-3">
                    <Spinner size="lg" />
                    <p className="text-sm text-surface-400">Loading shipment details…</p>
                </div>
            </div>
        </PageShell>
    );
        ...
        <PageShell>
            <div className="min-h-screen flex items-center justify-center p-6">
                <Card className="max-w-sm w-full p-8 text-center">
                    ...
                    <h1 className="text-lg font-semibold text-surface-900 mb-2">Tracking link not found</h1>
        ...
        <header className="bg-white border-b border-surface-100">
            <div className="max-w-lg mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2.5">
                    {businessLogo ? (
                        ...
                        <>
                            <div className="w-8 h-8 bg-surface-900 rounded-lg flex items-center justify-center shrink-0">
                                <span className="text-white text-sm font-black tracking-tighter">
                                    {businessName.charAt(0)}
                                </span>
                            </div>
                            <span className="text-base font-bold tracking-tight text-surface-900">{businessName}</span>
                        </>
                    )}
                    {businessTagline && !businessLogo && (
                        <span className="hidden sm:block text-xs text-surface-400 border-l border-surface-200 pl-2.5 ml-0.5">
                            {businessTagline}
                        </span>
                    )}
                </div>
            ...
            {businessTagline && businessLogo && (
                <div className="text-center pb-2 -mt-1">
                    <p className="text-xs text-surface-400">{businessTagline}</p>
                </div>
            )}
        </header>
    );
    ...
    return (
        <Card>
            <div className={clsx("px-5 py-4 flex items-start gap-3", {
                "bg-success-50":  isDelivered,
                "bg-danger-50":    isException || isAttempted,
                "bg-info-50":   showEta,
            })}>
                {/* Icon */}
                <div className={clsx("w-9 h-9 rounded-xl flex items-center justify-center shrink-0 mt-0.5", {
                    "bg-success-100": isDelivered,
                    "bg-danger-100":   isException || isAttempted,
                    "bg-info-100":  showEta,
                })}>
                    {isDelivered && (
                        <svg className="w-5 h-5 text-success-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                ...
                <div>
                    {isDelivered && (
                        <>
                            <p className="text-sm font-semibold text-success-800">Your order has been delivered</p>
                            {deliveredAt && (
...
export default function TrackingPage() {
    ...
    const barCls      = STATUS_CONFIG[shipment.status]?.bar ?? "bg-info-600";
        ...
        <PageShell>
            ...
            <main className="max-w-lg mx-auto px-4 py-6 pb-16 space-y-4">
                ...
                <Card>
                    <div className="px-5 py-4">
                        {shipment.customer_first_name && (
                            <p className="text-xs text-surface-400 mb-1">Hi {shipment.customer_first_name},</p>
                        )}
                    ...
                    <div className="px-5 py-4">
                        ...
                        {events.length > 0 ? (
                            ...
                            <div className="py-8 text-center">
                                ...
                                <p className="text-sm text-surface-400">No updates yet — check back soon.</p>
                            </div>
                        )}
                    </div>
                </Card>
                ...
                {shipment.carrier_tracking_url && (
                    <div className="flex justify-center">
                        <a
                            ...
                            className="inline-flex items-center gap-1.5 text-xs text-surface-400 hover:text-surface-700 transition-colors"
                        >
                ...
                <p className="text-center text-xs text-surface-300 flex items-center justify-center gap-1.5 pt-2">
                    ...
                    <a href="mailto:hello@bethanyhouse.co.ke" className="hover:text-surface-500 transition-colors">
